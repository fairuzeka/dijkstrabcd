	...
	*/
	public function drawRoute($shortest_path)
	{
		...
		for($i = 0; $i < (count($exp_shortest_path)-1); $i++){
			...
			$select = "SELECT jalur,bobot FROM graph where simpul_awal =" . $exp_shortest_path[$start] . " and simpul_tujuan =" . $exp_shortest_path[(++$start)];
			$query  = mysqli_query($this->koneksi, $select);			
			$fetch	= mysqli_fetch_array($query, MYSQLI_ASSOC);
			$jarak_tempuh += $fetch['bobot'];
		...
		// return
		$return_json = ['jalur_shortest_path'=>$semua_latlng, 'angkot'=>$angkot_array, 'bobot'=>$jarak_tempuh];		
